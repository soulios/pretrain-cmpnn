...
def replace_atom_smiles(smiles_batch, idx_replace, num_atom_type):
    batch = None
    while batch is None :
        # Create mol and replace the atom
        mol = Chem.MolFromSmiles(smiles_batch)
        # Replace the idx atom
        molw = Chem.RWMol(mol)
        molw.ReplaceAtom(idx_replace, Chem.Atom(random.randint(0,num_atom_type)))

        mol_replaced = Chem.MolToSmiles(molw)
        batch = Chem.MolFromSmiles(mol_replaced)
    ...
    def __call__(self, data, masked_atom_indices=None):
        ...
        * mask rate number of atom indices
        ...
        for atom_idx in masked_atom_indices:
            ...
            ## Replace smile
            data.smile_masked = replace_atom_smiles(smiles_batch, atom_idx, self.num_atom_type - 1)
            smiles_batch = data.smile_masked
