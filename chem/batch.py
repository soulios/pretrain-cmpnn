...
class BatchSubstructContext(Data):
    ...
    def from_data_list(data_list):
        ...
        b.batch_overlapped_context = []
        b.overlapped_context_size = []
        ...
        for data in data_list:
            ...
            if hasattr(data, "x_context"):
                ...
                #batch.batch.append(torch.full((num_nodes, ), i, dtype=torch.long))
                b.batch_overlapped_context.append(torch.full((len(data.overlap_context_substruct_idx), ), i, dtype=torch.long))
                b.overlapped_context_size.append(len(data.overlap_context_substruct_idx))
                ...
                for key in ["center_substruct_idx", "edge_attr_substruct", "edge_index_substruct", "x_substruct"]:
                    item = data[key]
                    item = item + cumsum_substruct if b.cumsum(key, item) else item
                    b[key].append(item)
                ...
                for key in ["overlap_context_substruct_idx", "edge_attr_context", "edge_index_context", "x_context"]:
                    item = data[key]
                    item = item + cumsum_context if b.cumsum(key, item) else item
                    b[key].append(item)
        ...
        for key in keys:
            b[key] = torch.cat(
                b[key], dim=b.cat_dim(key))
        #batch.batch = torch.cat(batch.batch, dim=-1)
        b.batch_overlapped_context = torch.cat(b.batch_overlapped_context, dim=-1)
        b.overlapped_context_size = torch.LongTensor(b.overlapped_context_size)
